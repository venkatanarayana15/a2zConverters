...
const PhysicsButton = ({ children, onClick, className = "", variant = "primary" }) => {
    ...
    const variants = {
        ...
        outline: "bg-white text-gray-900 border border-gray-200 hover:bg-gray-50 shadow-sm dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700 dark:hover:bg-primary/10 dark:hover:text-primary dark:hover:border-primary/30 dark:shadow-black/20",
        electric: "text-white shadow-lg shadow-cyan-500/30 overflow-hidden bg-slate-900 border border-transparent"
    };
        ...
        >
            {variant === 'electric' && (
                <>
                    ...
                    <div className="absolute inset-[2px] rounded-[10px] bg-slate-900 z-0" />
                </>
            )}
