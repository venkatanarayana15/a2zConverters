...
const getIconColor = (type) => {
    switch (type) {
        ...
        case 'security': return 'text-gray-600 dark:text-gray-400';
...
const getHoverColor = (type) => {
    switch (type) {
        ...
        case 'security': return 'hover:bg-gray-100 dark:hover:bg-gray-800';
    ...
    return (
        <div className="w-full bg-white/95 dark:bg-gray-900/95 backdrop-blur-3xl border-t border-gray-100 dark:border-gray-700 shadow-2xl overflow-y-auto max-h-[85vh]">
            <div className="max-w-[96rem] mx-auto px-6 py-5">
                ...
                {activeSection === 'pdf' && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 pb-3 border-b border-gray-100 dark:border-gray-700">
                            ...
                            <h2 className="text-sm font-bold text-gray-900 dark:text-gray-100 tracking-wide">PDF Tools</h2>
                        </div>

                        <div className="grid grid-cols-5 gap-6">
                            {pdfCategories.map((category, idx) => (
                                <div key={idx} className="space-y-3">
                                    <h3 className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider pl-1.5">
                                        {category.title}
                                    </h3>
                                        ...
                                        {category.paths.map((path) => {
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-[13px] font-medium text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-100 leading-tight">
                                                        {tool.name}
                                                    </span>
                                                    <span className="text-[10px] text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-400 leading-tight mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity h-0 group-hover:h-auto overflow-hidden">
                                                        {tool.desc}
                                                    </span>
                                                </div>
                                            </Link>
                ...
                {activeSection === 'image' && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-center gap-2 pb-3 border-b border-gray-100 dark:border-gray-700">
                            <div className="p-1.5 rounded-lg bg-purple-50 dark:bg-purple-900/20 text-purple-600">
                                <ImageIcon className="w-4 h-4" />
                            </div>
                            <h2 className="text-sm font-bold text-gray-900 dark:text-gray-100 tracking-wide">Image Tools</h2>
                        </div>
                            ...
                            {imageCategories[0].paths.map((path) => {
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={cn("p-2 shrink-0 rounded-full bg-white dark:bg-gray-800 shadow-sm group-hover:scale-105 transition-transform duration-300", getIconColor('image'))}>
                                            <Icon className="w-5 h-5" />
                                        </div>
                                        <div>
                                            <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-gray-100 block">
                                                {tool.name}
                                            </span>
                                            <span className="text-[10px] text-gray-400 dark:text-gray-500 group-hover:text-gray-500 dark:group-hover:text-gray-400 leading-tight block">
                                                {tool.desc}
                                            </span>
                                        </div>
                                    </div>
