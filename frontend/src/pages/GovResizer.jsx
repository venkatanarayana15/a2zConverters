        ...
        <div className="min-h-screen pt-24 px-4 pb-12 bg-background text-foreground">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-12 animate-float">
                    ...
                    <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="glass-card p-8 rounded-2xl relative overflow-hidden group">
                        ...
                        <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 ${file ? 'border-orange-500 bg-orange-50' : 'border-gray-300 hover:border-orange-400 hover:bg-orange-50/50'}`}>
                            <input
                                type="file"
                                id="file-upload"
                                className="hidden"
                            ...
                            <label htmlFor="file-upload" className="cursor-pointer w-full h-full flex flex-col items-center justify-center">
                                {preview ? (
                                    ...
                                    <>
                                        <div className="w-16 h-16 bg-white border border-gray-100 shadow-sm rounded-full flex items-center justify-center mb-4 text-gray-400 group-hover:scale-110 group-hover:text-orange-500 transition-all duration-300">
                                            <Upload className="w-8 h-8" />
                                        </div>
                                        <span className="text-gray-900 font-medium">Click to upload photo</span>
                    ...
                    <div className="glass-card p-8 rounded-2xl relative overflow-hidden">
                        ...
                        <div className="space-y-6">
                            <div ref={dropdownRef} className="relative">
                                <label className="block text-sm font-medium text-gray-700 mb-2">Select Exam / Purpose</label>

                                {/* Custom Dropdown Trigger */}
                                <button
                                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                                    className="w-full bg-white/80 border border-gray-200 rounded-xl px-4 py-3 text-left text-gray-900 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all flex justify-between items-center shadow-sm hover:bg-white"
                                >
                                ...
                                {/* Dropdown Menu */}
                                {isDropdownOpen && (
                                    <div className="absolute top-full left-0 w-full mt-2 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-20 animate-fade-in-down">
                                        <div className="p-2 border-b border-gray-50 sticky top-0 bg-white z-10">
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-gray-50 border border-gray-200 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-sky-500"
                                                    autoFocus
                                                />
                                            </div>
                                        </div>
                                            ...
                                            {filteredPresets.length > 0 ? (
                                                filteredPresets.map(([key, preset]) => (
                                                    <button
                                                        ...
                                                        className={`w-full text-left px-4 py-3 text-sm hover:bg-sky-50 transition-colors flex items-center justify-between ${selectedExam === key ? 'bg-sky-50 text-sky-700 font-medium' : 'text-gray-700'}`}
                                                    >
                                                        {preset.label}
                            ...
                            <div>
                                ...
                                <div className="flex space-x-4 bg-gray-50 p-1.5 rounded-xl border border-gray-200 w-fit">
                                    {['px', 'cm', 'inch'].map((u) => (
                                        <button
                                            key={u}
                                            onClick={() => setUnit(u)}
                                            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${unit === u
                                                    ? 'bg-white text-sky-600 shadow-sm ring-1 ring-gray-100'
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/80 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 focus:outline-none focus:border-sky-500 transition-all"
                ...
                <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-6 rounded-2xl bg-white/60 border border-white/50 shadow-sm text-center hover:bg-white/80 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center mx-auto mb-4">
                            <Check className="w-6 h-6" />
                        </div>
                        <h3 className="font-bold text-gray-900 mb-2">Exact Dimensions</h3>
                        <p className="text-sm text-gray-500">Guaranteed width x height compliance for pixels or centimeters.</p>
                    </div>
                    <div className="p-6 rounded-2xl bg-white/60 border border-white/50 shadow-sm text-center hover:bg-white/80 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mx-auto mb-4">
                            <Info className="w-6 h-6" />
                        </div>
                        <h3 className="font-bold text-gray-900 mb-2">File Size Control</h3>
                        <p className="text-sm text-gray-500">Compresses your image to fit strictly under the KB limit (e.g. &lt;50KB).</p>
                    </div>
                    <div className="p-6 rounded-2xl bg-white/60 border border-white/50 shadow-sm text-center hover:bg-white/80 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center mx-auto mb-4">
                            <AlertCircle className="w-6 h-6" />
                        </div>
                        <h3 className="font-bold text-gray-900 mb-2">Privacy First</h3>
                        <p className="text-sm text-gray-500">Images are processed in your browser. We never store your personal photos.</p>
                    </div>
                </div>
            </div>
        </div>
