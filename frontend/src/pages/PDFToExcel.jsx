...
import { Upload, Table2, CheckCircle, Download } from 'lucide-react';
...
const PDFToExcel = () => {
    ...
    const handleConvert = async () => {
        ...
        try {
            const res = await apiPost('/api/v1/pdf/pdf-to-excel', makeUploadForm(file, {}, 'file'));
            setResult(res);
        } catch (error) {
            alert(error.message);
        } finally {
        ...
        <div className="min-h-screen pt-24 px-4 pb-12 bg-gray-50 text-gray-900">
            <div className="max-w-3xl mx-auto">
                <div className="text-center mb-10">
                    <div className="inline-flex items-center px-3 py-1 rounded-full bg-green-50 border border-green-200 text-sm font-medium text-green-600 mb-4">
                        <Table2 className="w-4 h-4 mr-2" />
                        PDF to Excel
                    </div>
                    <h1 className="text-4xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-green-700 to-emerald-500">
                        Convert PDF to Excel
                    </h1>
                    <p className="text-gray-600">
                        Extract the text from your PDF into an XLSX spreadsheet, one sheet per page.
                    </p>
                </div>
                ...
                <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-10 text-center">
                    {!file ? (
                        <div className="border-2 border-dashed border-gray-200 rounded-xl p-16 hover:bg-gray-50 transition-colors">
                            <input type="file" accept=".pdf" onChange={handleFileChange} className="hidden" id="pdf-excel-upload" />
                            <label htmlFor="pdf-excel-upload" className="cursor-pointer">
                        ...
                        <div className="py-10">
                            <div className="mb-8">
                                <Table2 className="w-16 h-16 text-green-600 mx-auto mb-4" />
