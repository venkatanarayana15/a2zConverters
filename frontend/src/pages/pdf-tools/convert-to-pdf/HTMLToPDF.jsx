        ...
        <ToolLayout icon={Globe} badge="Convert to pdf" title="HTML to PDF" subtitle="Paste any HTML content and download it as a clean PDF." accent="red">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-8 dark:bg-gray-900 dark:border-gray-800 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">HTML content</label>
                        <textarea
                            value={html}
                            onChange={(e) => setHtml(e.target.value)}
                            rows={14}
                            spellCheck={false}
                            className="w-full bg-white/50 border border-gray-200 rounded-lg px-4 py-3 font-mono text-xs focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-all dark:bg-gray-800/50 dark:border-gray-700 dark:text-gray-100"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Live preview</label>
                        <div className="rounded-xl border border-gray-200 overflow-hidden bg-white dark:border-gray-700">
                            <div className="px-4 py-3 text-sm dark:bg-gray-100" ref={renderRef} dangerouslySetInnerHTML={{ __html: html }} />
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Page size</label>
                        <div className="grid grid-cols-2 gap-2 p-1 bg-gray-100 rounded-xl dark:bg-gray-800">
                            {['a4', 'letter'].map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFormat(f)}
                                    className={`py-2 rounded-lg text-sm font-medium capitalize transition-all ${format === f ? 'bg-white shadow-sm text-red-600 dark:bg-gray-700' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                                >
                                    {f}
                                </button>
                            ))}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Orientation</label>
                        <div className="grid grid-cols-2 gap-2 p-1 bg-gray-100 rounded-xl dark:bg-gray-800">
                            {['portrait', 'landscape'].map((o) => (
                                <button
                                    key={o}
                                    onClick={() => setOrientation(o)}
                                    className={`py-2 rounded-lg text-sm font-medium capitalize transition-all ${orientation === o ? 'bg-white shadow-sm text-red-600 dark:bg-gray-700' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                                >
                                    {o}
                                </button>
                            ))}
