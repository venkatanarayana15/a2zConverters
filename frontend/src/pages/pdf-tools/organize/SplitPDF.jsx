    ...
    return (
        <ToolLayout icon={Scissors} badge="Organize" title="Split PDF" subtitle="Split a PDF into separate files — one page each, or by custom page ranges." accent="red">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-8 dark:bg-gray-900 dark:border-gray-800 space-y-6">
                ...
                {pageCount > 1 && (
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Split Mode</label>
                        <div className="grid grid-cols-2 gap-2 p-1 bg-gray-100 rounded-xl dark:bg-gray-800">
                            <button
                                onClick={() => setMode('single')}
                                className={`py-2 rounded-lg text-sm font-medium transition-all ${mode === 'single' ? 'bg-white shadow-sm text-red-600 dark:bg-gray-700' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                One page per file
                            </button>
                            <button
                                onClick={() => setMode('ranges')}
                                className={`py-2 rounded-lg text-sm font-medium transition-all ${mode === 'ranges' ? 'bg-white shadow-sm text-red-600 dark:bg-gray-700' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                            >
                                Custom ranges
                            </button>
                        </div>
                        ...
                        {mode === 'ranges' && (
                            <div className="mt-4">
                                <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Page Ranges</label>
                                <input
                                    type="text"
                                    value={rangesInput}
                ...
                {result && (
                    <div className="p-4 rounded-xl bg-green-50 border border-green-200 dark:bg-green-900/20 dark:border-green-800">
                        <p className="font-bold text-green-700 dark:text-green-400 flex items-center gap-2"><Download className="w-4 h-4" /> Split complete</p>
                        <p className="text-sm text-green-600 dark:text-green-500">{result.count} files created ({formatBytes(result.total)})</p>
                    </div>
                )}
