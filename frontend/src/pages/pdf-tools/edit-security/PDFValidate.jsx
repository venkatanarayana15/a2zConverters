            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="glass-card p-8 rounded-2xl relative overflow-hidden group md:col-span-2 lg:col-span-1">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-blue-100 rounded-full blur-3xl -z-10 transition-all group-hover:bg-blue-200" />
                        ...
                        <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 ${file ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 hover:border-blue-400 hover:bg-blue-50/50 dark:border-gray-600 dark:hover:bg-blue-900/10'}`}>
                            <input
                                type="file"
                                id="pdf-upload"
                                className="hidden"
                            ...
                            <label htmlFor="pdf-upload" className="cursor-pointer w-full h-full flex flex-col items-center justify-center">
                                {file ? (
                                    <div className="flex flex-col items-center">
                                        ...
                                        <span className="text-gray-900 font-medium dark:text-gray-100">{file.name}</span>
                                        <span className="text-sm text-gray-500 mt-1 dark:text-gray-400">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
                                        <button
                                    ...
                                    <>
                                        <div className="w-16 h-16 bg-white border border-gray-100 shadow-sm rounded-full flex items-center justify-center mb-4 text-gray-400 group-hover:scale-110 group-hover:text-blue-500 transition-all duration-300 dark:bg-gray-800 dark:border-gray-700">
                                            <Upload className="w-8 h-8" />
                                        </div>
                                        <span className="text-gray-900 font-medium dark:text-gray-100">Click to upload PDF</span>
                                        <span className="text-sm text-gray-500 mt-2 dark:text-gray-400">PDF documents only</span>
                                    </>
                                )}
                            </label>
                        </div>
                    ...
                    <div className="glass-card p-8 rounded-2xl relative overflow-hidden flex flex-col justify-center min-h-[400px]">
                        <div className="absolute bottom-0 left-0 w-32 h-32 bg-indigo-100 rounded-full blur-3xl -z-10" />

                        {!validationResult ? (
                            <div className="text-center text-gray-500 dark:text-gray-400">
                                <ShieldCheck className="w-16 h-16 mx-auto mb-4 text-gray-300 dark:text-gray-600" />
                            ...
                            <div className="animate-fade-in">
                                <div className={`flex items-center justify-center w-20 h-20 rounded-full mx-auto mb-6 ${validationResult.isValid ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                    {validationResult.isValid ? <CheckCircle className="w-10 h-10" /> : <AlertTriangle className="w-10 h-10" />}
                                </div>
                                <h3 className="text-2xl font-bold text-center text-gray-900 mb-2 dark:text-gray-100">
                                ...
                                <div className="space-y-3">
                                    {validationResult.details.map((detail, idx) => (
                                        <div key={idx} className="flex items-center p-3 bg-white/50 rounded-lg border border-gray-100 dark:bg-gray-800/50 dark:border-gray-700">
                                            <CheckCircle className="w-5 h-5 text-green-500 mr-3 flex-shrink-0" />
                                            <span className="text-gray-700 text-sm dark:text-gray-300">{detail}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
